...
下载并解压 `recallforge-skill-v2.1.0.zip`。在文件资源管理器地址栏输入 `%USERPROFILE%\.agents\skills`，按需要创建文件夹，将解压得到的 `recallforge` 文件夹复制进去。最终必须是：`%USERPROFILE%\.agents\skills\recallforge\SKILL.md`。
