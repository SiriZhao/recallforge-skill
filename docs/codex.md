...
Download and extract `recallforge-skill-v2.1.0.zip`. In File Explorer, type `%USERPROFILE%\.agents\skills` in the address bar (create the folders if asked), then copy the extracted `recallforge` folder there. The final file must be `%USERPROFILE%\.agents\skills\recallforge\SKILL.md`.
